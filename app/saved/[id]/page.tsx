...
import CollectionShareControls from './_components/CollectionShareControls';
import CollectionManageControls from './_components/CollectionManageControls';
import RemoveToolButton from './_components/RemoveToolButton';
...
export default async function CollectionPage({
  ...
  if (!user) redirect(`/login?redirectTo=/saved/${id}`);

  const admin = createAdminClient();
  const { data: collection } = await admin
    .from('collections')
    .select(`
      id, name, description, is_public, share_token, created_at, user_id,
      collection_tools (
        id, added_at,
        tools (
  ...
  const tools = ((collection.collection_tools as any[]) || [])
    .map((ct: any) => ({ ...ct.tools, ct_id: ct.id }))
    .filter(Boolean);

  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://currly-beta.vercel.app';
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-[#050505] text-[#1A1A1A] dark:text-white font-sans">
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        {/* Header */}
        <div className="bg-white dark:bg-[#111] rounded-3xl border border-gray-100 dark:border-white/5 p-6 mb-8">
          <CollectionManageControls
          ...
          <div className="mt-5 pt-5 border-t border-gray-100 dark:border-white/5">
            <CollectionShareControls
              ...
              initialShareToken={collection.share_token ?? null}
              siteUrl={siteUrl}
            />
          </div>
        </div>
        ...
        {tools.length === 0 ? (
          <div className="text-center py-24">
            <p className="text-gray-400 mb-4">No tools in this collection yet.</p>
            <Link
              href="/dashboard"
              className="bg-[#0066FF] text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-[#0052CC] transition-colors"
            >
              Browse Tools
            </Link>
          </div>
